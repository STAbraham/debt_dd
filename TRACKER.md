...
**Status:** v17 — all 14 initial-doc items closed. Live work is the 8/26 email batch, now items 6–13: Drafted 6.1, 6.2, 7, 9.1–9.3, 12.1, 13.2 · Open 8, 10, 11, 12.2, 13.1, 13.3. Installment Funds Flow diagram staged in Data Room / Product (Box upload pending).
...
**A:** To be provided.
**Note (internal):** Fund email 2026-08-26, bullet 6. **Interchange revenue is NOT in the warehouse** — verified 2026-09-03 (see METHODS.md): no interchange/MDR/settlement-fee column in any of the 62 tables; `public_transactions` and `public_network_messages` carry only gross transaction amounts. The warehouse mirrors the app DB, and interchange never posts there — it lives in i2c/Mastercard settlement reporting and finance records. So this answer needs a finance-side number for monthly net interchange (net of network + processor costs); the GMV denominator can come from BQ once Steve confirms the base (posted purchase volume?). Domestic/international split depends on what the settlement reports break out. [Steve: where does net interchange actually get tracked — i2c settlement reports, the bank recon, finance's books? And who has it — Savannah/finance?]
